  ...
  return (
    <>
      ...
      <div className="mb-16 reveal-fade">
        <Link href="/projects" className="inline-flex items-center gap-2 text-sm text-white/50 hover:text-white transition-colors uppercase font-mono tracking-widest">
      ...
      <header className="mb-16 md:mb-24 reveal-fade max-w-5xl">
        ...
        <p className="text-sm md:text-base text-white/50 leading-relaxed max-w-3xl font-mono normal-case tracking-normal">
      ...
      <section className="mb-20 md:mb-32 grid md:grid-cols-12 gap-12 reveal-fade">
        ...
        <div className="md:col-span-8 prose prose-invert font-serif normal-case tracking-normal text-lg md:text-xl leading-relaxed text-white/80">
          ...
          <p className="text-white font-medium">El problema real era reducir la carga operativa de negocios locales que tienen cero infraestructura digital y muy poco tiempo para aprender herramientas nuevas.</p>
        </div>
      </section>
          ...
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-12">
            {constraints.map((c, i) => (
              <div key={i} className="border-l border-white/20 pl-4 relative group">
                ...
                <p className="text-sm text-white/60 font-mono normal-case tracking-normal leading-relaxed">{c.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mb-20 md:mb-32 reveal-fade">
        ...
        <div className="bg-black/40 border border-white/10 p-4 md:p-8 rounded-sm overflow-x-auto relative group">
          ...
          <pre className="font-mono text-[10px] md:text-xs text-white/60 leading-tight">
{`┌──────────────────────────────────────────────────────┐
│                   Vercel (Serverless)                │
│                                                      │
...
└───────────┘          └───────────────┘`}
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 font-mono text-xs text-white/50 pt-4 border-t border-white/10">
            <div><span className="text-white/80">ADMIN:</span> Global management</div>
            <div><span className="text-white/80">OWNER:</span> Full control</div>
            <div><span className="text-white/80">STAFF:</span> Personal agenda</div>
            <div><span className="text-white/80">CLIENT:</span> Booking</div>
          </div>
        </div>
      </section>
      ...
      <section className="mb-20 md:mb-32 reveal-fade">
        ...
        <div className="flex flex-col gap-8 text-sm normal-case font-mono tracking-normal">
          {decisions.map((d, i) => (
            <div key={i} className="grid md:grid-cols-12 gap-6 bg-white/[0.02] border border-white/10 p-6 md:p-8 hover:bg-white/[0.04] transition-colors">
              <div className="md:col-span-4 flex flex-col justify-between">
                <span className="text-xs text-white/40 uppercase tracking-widest mb-4 inline-block">{d.topic}</span>
                <span className="text-base text-white/90 font-medium">{d.decision}</span>
              </div>
              <div className="md:col-span-8 flex flex-col gap-4">
                <div>
                  <h4 className="text-[10px] uppercase text-white/30 tracking-widest mb-1">Why</h4>
                  <p className="text-white/70 leading-relaxed">{d.why}</p>
                </div>
                <div className="border-t border-white/10 pt-4 mt-2">
                  <h4 className="text-[10px] uppercase text-white/30 tracking-widest mb-1">Trade-off Accepted</h4>
                  <p className="text-white/50">{d.tradeoff}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="mb-20 md:mb-32 grid md:grid-cols-12 gap-12 reveal-fade">
        ...
        <div className="md:col-span-8">
          <div className="flex flex-col gap-6">
            {lessons.map((l, i) => (
              <div key={i} className="border border-white/10 p-6 flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-white/80">{l.title}</h3>
                  <p className="text-sm font-mono tracking-normal normal-case text-white/50">{l.issue}</p>
                </div>
                <div className="bg-white/5 p-4 rounded-sm border-l-2 border-white/50">
                  <p className="text-sm font-mono normal-case tracking-normal text-white/80"><span className="text-[10px] uppercase text-white/40 mr-2 tracking-widest">Lesson:</span>{l.lesson}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="grid md:grid-cols-2 gap-12 mb-24 reveal-fade pt-12 border-t border-white/10">
        <div>
          <h2 className="text-xs text-white/50 font-mono tracking-widest uppercase mb-6">06 // Outcome</h2>
          <ul className="text-sm font-mono normal-case tracking-normal text-white/70 flex flex-col gap-3">
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> TimeUp está en producción activa bajo timeup.mx</li>
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Flujos completos de onboarding, operación y reserva pública</li>
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Cron billing diario en producción estable</li>
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Dashboard Time-to-First-Byte (TTFB) &lt; 500 ms</li>
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Arquitectura extensible sin refactor estructural post-MVP</li>
          </ul>
        </div>
        <div>
          <h2 className="text-xs text-white/50 font-mono tracking-widest uppercase mb-6">07 // What I Would Do Differently</h2>
          <ul className="text-sm font-mono normal-case tracking-normal text-white/70 flex flex-col gap-3">
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Simplificaría drásticamente el modelo de usuarios/roles.</li>
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Extraería una capa de servicios explícita desde el día uno.</li>
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Implementaría un entorno de staging obligatorio para CI/CD.</li>
            <li className="flex items-start gap-3"><span className="text-white/30">→</span> Usaría un proveedor gestionado para realtime (Pusher/Supabase RT) en lugar de WebSockets custom.</li>
          </ul>
        </div>
      </section>

      <div className="text-center md:text-left text-[10px] text-white/30 font-mono tracking-widest uppercase pt-12 border-t border-white/10 reveal-fade">
